...
func (r *ReconcileSP) Reconcile(_ context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	nodeStatus, err := nodestatus.NewForPolicy(instance)
	if err != nil {
	...
	statusCopy.State = secpolnodestatusv1alpha1.ProfileStateTerminating
...
func (r *ReconcileSP) ensureNodeStatus(
	...
	if sp.Status.State == "" {
		...
		policyCopy.Status.State = secpolnodestatusv1alpha1.ProfileStatePending
...
func (r *ReconcileSP) reconcilePolicy(
	...
	if polStatus == nil {
		...
		statusCopy.State = secpolnodestatusv1alpha1.ProfileStateInProgress
	...
	case installedStatus:
		statusCopy.State = secpolnodestatusv1alpha1.ProfileStateInstalled
		statusCopy.SetConditions(rcommonv1.Available())
	case failedStatus:
		statusCopy.State = secpolnodestatusv1alpha1.ProfileStateError
