...
import (
	...
	secpolnodestatusv1alpha1 "sigs.k8s.io/security-profiles-operator/api/secpolnodestatus/v1alpha1"
...
func NewForPolicy(pol client.Object) (*StatusClient, error) {
...
func (nsf *StatusClient) statusObj(
	polState secpolnodestatusv1alpha1.ProfileState,
) *secpolnodestatusv1alpha1.SecurityProfileNodeStatus {
	return &secpolnodestatusv1alpha1.SecurityProfileNodeStatus{
		ObjectMeta: metav1.ObjectMeta{
			Name:      nsf.perNodeStatusName(),
			Namespace: nsf.pol.GetNamespace(),
			Labels: map[string]string{
				secpolnodestatusv1alpha1.StatusToPolLabel:  nsf.pol.GetName(),
				secpolnodestatusv1alpha1.StatusToNodeLabel: nsf.nodeName,
				secpolnodestatusv1alpha1.StatusStateLabel:  string(polState),
				secpolnodestatusv1alpha1.StatusKindLabel:   nsf.pol.GetObjectKind().GroupVersionKind().Kind,
			},
		},
...
func (nsf *StatusClient) createNodeStatus(ctx context.Context, c client.Client) error {
	err := c.Create(ctx, nsf.statusObj(secpolnodestatusv1alpha1.ProfileStatePending))
...
func (nsf *StatusClient) removeNodeStatus(ctx context.Context, c client.Client) error {
	...
	err := c.Delete(ctx, nsf.statusObj(secpolnodestatusv1alpha1.ProfileStateTerminating))
...
func (nsf *StatusClient) nodeStatusExists(ctx context.Context, c client.Client) (bool, error) {
	status := secpolnodestatusv1alpha1.SecurityProfileNodeStatus{}
...
func (nsf *StatusClient) SetReturnGlobal(
	ctx context.Context, c client.Client, polState secpolnodestatusv1alpha1.ProfileState,
) (secpolnodestatusv1alpha1.ProfileState, error) {
	if err := nsf.setNodeStatus(ctx, c, polState); err != nil {
		return secpolnodestatusv1alpha1.ProfileStateError, errors.Wrap(err, "setting node status")
	}

	// search all statuses
	statusList := secpolnodestatusv1alpha1.SecurityProfileNodeStatusList{}
	allStatusesForPol := client.MatchingLabels{
		secpolnodestatusv1alpha1.StatusToPolLabel: nsf.pol.GetName(),
	}
	if err := c.List(ctx, &statusList, allStatusesForPol); err != nil {
		return secpolnodestatusv1alpha1.ProfileStateError, errors.Wrap(err, "listing statuses")
	}

	// figure out the 'lowest common one'
	lowestCommonState := secpolnodestatusv1alpha1.ProfileStateTerminating
	for i := range statusList.Items {
		lowestCommonState = secpolnodestatusv1alpha1.LowerOfTwoStates(lowestCommonState, statusList.Items[i].Status)
	}
...
func (nsf *StatusClient) setNodeStatus(
	...
	status := secpolnodestatusv1alpha1.SecurityProfileNodeStatus{}
	err := c.Get(ctx, nsf.perNodeStatusNamespacedName(), &status)
	if kerrors.IsNotFound(err) && polState == secpolnodestatusv1alpha1.ProfileStateTerminating {
		// it's OK if we're about to terminate a policy but it was already gone
		return nil
	} else if err != nil {
	...
	status.Labels[secpolnodestatusv1alpha1.StatusStateLabel] = string(polState)
